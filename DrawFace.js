...
class DrawFace
{
	constructor(canvas_id, hair_color, eye_size, eye_position)
	{
		...
		*************************************************************/
		...
		this.eyelid_bottom = [
			{x: this.eye_head[0].x + this.eye_position, y: this.center.y + eye_size +5}, 
			{x: this.eye_head[1].x - this.eye_position, y: this.center.y + eye_size +5}, 
		];
	...
	draw()
	{
		...
		this.drawDebug();
		this.drawOutsideBackHair(8, 400);
		this.drawBackHair(16, 400);
		this.drawNeck();
		this.drawNeckShadow();
		this.drawEar();
		this.drawOutline();
		...
		this.drawSkinHead();
		//this.drawFrontHair3(0, 12, 110);
		this.drawFrontHair2(10, 100);
		//this.drawSideHair3(180, 3);
		this.drawSideHair2(180, 3);
		this.drawOutsideHair(6, 200);
		/*
		this.drawCatalinaHair(400, 1/2, 5, 10);
		this.drawCatalinaHair(400, 1/2, 5, 10, RIGHT);
		this.drawFrontline(400, 10, 1/2)
		this.drawFrontline(400, 10, 1/2, RIGHT)
		*/
	}
	...
	drawSideHair2(hair_length, hair_bunch)
	{
		...
		for (let j=0; j<4; j++)
		{
			...
			/* 左サイド髪の描画 */
			this.con.beginPath();
			//this.con.moveTo(side_hair_left.x, side_hair_left.y);
	...
	drawFrontline(length, hair_bunch, split, direction = LEFT)
	{
		...
		for (let j=0; j<=0; j++)
		{
			...
			for (let i=0; i<=hair_bunch; i++)
			{
				...
				front_tips[i] = {
					x: front_tip_start.x + i*front_tip_span*direction + rand(-180, 0),
					y: front_tip_start.y,
				};
	...
	drawFrontHair3(type = 0, hair_bunch, hair_length)
	{
	...
	drawLowerEyelid()
	{
		for (let i=0; i<2; i++)
		{
			...
			this.con.beginPath();
			this.drawCurve2(this.upper_eyeline_end[i], this.eye_head[i], this.lower_eyelid_cp4[i], this.lower_eyelid_cp3[i], true);
			this.drawCurve(this.eye_head[i], this.eyelid_bottom[i], this.eyelid_bottom_cp1[i] );
			this.drawCurve(this.eyelid_bottom[i], this.eye_end[i], this.eyelid_bottom_cp3[i] );
			this.con.lineTo(this.upper_eyeline_end[i].x, this.upper_eyeline_end[i].y);
			this.con.fill();
		}
	}
	...
	drawEyelid()
	{
		for (let i=0; i<2; i++)
		{
			...
			this.drawCurve2(this.upper_eyeline_end[i], this.eye_head[i], this.eyelid_cp2[i], this.eyelid_cp1[i]);
			this.con.fill();
		}
	}
	...
	drawIris()
	{
		...
		for (let i=0; i<2; i++)
		{
			...
			this.con.beginPath();
			this.con.arc(
				this.eye_head[i].x + this.eye_position * pn, this.center.y, 
				this.eye_size,  
				Math.PI * 2, false);
			this.con.restore();
			this.con.fill();
			...
			this.con.beginPath();
			this.con.arc(
				this.eye_head[i].x + this.eye_position * pn,
				this.center.y, 
				7,  
				Math.PI * 2, false);
			this.con.restore();
			...
			this.con.beginPath();
			this.con.arc(
				this.eye_head[i].x+ this.eye_position *pn + 10 + eye_position_rand , 
				this.center.y -10, 
				5,  
				Math.PI * 2, false);
			this.con.arc(
				this.eye_head[i].x + this.eye_position *pn + 4  + eye_position_rand, 
				this.center.y -5, 
				2, 
				Math.PI * 2, false);
			this.con.restore();
